...
# Carregar imagens
IMAGEM_CANO = pygame.transform.scale2x(pygame.image.load(os.path.join('img_jogo', 'pipe.png')))
IMAGEM_CHAO = pygame.transform.scale2x(pygame.image.load(os.path.join('img_jogo', 'base.png')))
IMAGEM_BACKGROUND = pygame.transform.scale2x(pygame.image.load(os.path.join('img_jogo', 'bg.png')))
IMAGENS_PASSARO = [
    pygame.transform.scale2x(pygame.image.load(os.path.join('img_jogo', 'bird1.png'))),
    pygame.transform.scale2x(pygame.image.load(os.path.join('img_jogo', 'bird2.png'))),
    pygame.transform.scale2x(pygame.image.load(os.path.join('img_jogo', 'bird3.png'))),
]
...
class Passaro:
    ...
    def desenhar(self, tela):
        ...
        if self.contagem_image < self.TEMPO_ANIMACAO:
            self.image = self.IMGS[0]
        elif self.contagem_image < self.TEMPO_ANIMACAO * 2:
            self.image = self.IMGS[1]
        elif self.contagem_image < self.TEMPO_ANIMACAO * 3:
            self.image = self.IMGS[2]
        elif self.contagem_image < self.TEMPO_ANIMACAO * 4:
            self.image = self.IMGS[1]
        elif self.contagem_image >= self.TEMPO_ANIMACAO * 4 + 1:
            self.image = self.IMGS[0]
            self.contagem_image = 0

        if self.angulo <= -80:
            self.image = self.IMGS[1]
            self.contagem_image = self.TEMPO_ANIMACAO * 2

        imagem_rotacionada = pygame.transform.rotate(self.image, self.angulo)
        posicao_centro_imagem = self.image.get_rect(topleft=(self.x, self.y)).center
